...
const normalizeInvoice = (row = {}) => ({
  ...
  medicineName: row.medicine_name || row.medicineName || row.item_name || row.prescription_name || row.store_name || "Medication",
  medicalStore: row.store_name || row.medical_store_name || row.name || row.medicalStore || "Medical Store",
  ...
  invoiceNumber: row.invoice_number || row.invoiceNo || row.reference_no || `MED-${row.connection_id || row.id || Date.now()}`,
});
...
export default function DoctorMedicalPanel() {
  ...
  const [tableFilters, setTableFilters] = useState({ search: "", medicalStore: "", status: "", date: "" });
  ...
  const filteredRows = useMemo(() => {
    ...
    return invoiceRows.filter((row) => {
      ...
      const matchesStore = !tableFilters.medicalStore || row.medicalStore === tableFilters.medicalStore;
  ...
  const handleFilter = (field) => (value) => {
    const nextValue = value && typeof value === "object" && "target" in value ? value.target.value : value;

    setTableFilters((current) => ({ ...current, [field]: nextValue }));
    if (field !== "search") setTablePage(1);
  };

  const resetFilters = () => {
    setTableFilters({ search: "", medicalStore: "", status: "", date: "" });
    setTablePage(1);
  };

  return (
    <Box sx={{ mt: { xs: 7, md: 8 }, display: "grid", gap: 3 }}>
      <SectionTitle title="Medical Requests" description="Track your patient medical requests, preferred store, and invoice details in one place." />

      <Box className="lab-workspace__filter-bar">
        <Typography sx={{ color: "#123f66", fontWeight: 800, fontSize: "0.68rem", letterSpacing: "0.06em", mr: 0.5 }}>
          FILTERS
        </Typography>
          ...
          InputProps={{
            startAdornment: (
              <InputAdornment position="start">
                <SearchIcon sx={{ color: "#789096", fontSize: 18 }} />
              </InputAdornment>
            ),
          }}
        />
        ...
        <TextField
          ...
          onChange={(event) => handleFilter("medicalStore")(event.target.value)}
          sx={{ minWidth: { md: 200 } }}
        >
        ...
        <TextField
          ...
          onChange={(event) => handleFilter("date")(event.target.value)}
          InputLabelProps={{ shrink: true }}
          sx={{ minWidth: { md: 170 } }}
        />

        <Button variant="text" sx={{ color: "#0b5c8e", fontWeight: 700 }} onClick={resetFilters}>
          Reset
        </Button>
      </Box>
        ...
        {visibleRows.length ? (
          visibleRows.map((row, index) => (
            <TableRow key={row.id} hover>
              <TableCell sx={{ color: "#1f2937 !important", fontWeight: 600 }}>
              ...
              <TableCell sx={{ color: "#1f2937 !important", maxWidth: 210, whiteSpace: "normal" }}>
                {row.medicineName}
              </TableCell>
